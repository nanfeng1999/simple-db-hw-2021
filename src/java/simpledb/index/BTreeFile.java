...
public class BTreeFile implements DbFile {
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        updateParentPointer(tid, dirtypages, parentInternalPage.getId(), newPage.getId());
    ...
    private void handleMinOccupancyLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page,
        ...
        // 最多可以空多少个tuple 503 最多空252个tuple
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        // 获取兄弟的迭代器
        Iterator<Tuple> siblingIter;
        if(isRightSibling){
            // 偷右边的节点 因为右边的节点要大于父节点 所以偷了之后 父节点需要修改它的数值 修改为右兄弟被偷的节点
        ...
        while(siblingIter.hasNext() && stealNum > 0){
            lastTuple = siblingIter.next();
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // 计算需要移动的entry
        int stealNum = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
        if(stealNum <= 0){
            return;
        }
        ...
        while(leftReverseIter.hasNext() && stealNum > 0){
            BTreeEntry entry = leftReverseIter.next();
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // the corresponding parent entry. Be sure to update the parent
        // pointers of all children in the entries that were moved.
        int stealNum = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
        if(stealNum <= 0){
            return;
        }
